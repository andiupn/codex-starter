...
La cartella ini menyimpan hasil riset yang bisa dipakai ulang pada task berikutnya.
...
- `index.json`: catalogo ringkas semua riset
- `entries/<id>/report.md`: ringkasan riset yang siap dibaca ulang
- `entries/<id>/sources.json`: daftar sumber untuk riset tersebut

## Il flusso di lavoro non è corretto

1. Gialankanese `./scripts/project-health.sh --auto`
...
4. Struttura di audit con `python3 scripts/research-health.py`
5. Jika ada insight yang sangat stabil, promuovi ringkasannya ke `.codex-memory/`

## Ricerca Beda vs memoria

- `research/`: untuk hasil riset yang masih butuh sumber, konteks, dan bisa dipakai ulang nanti
- `.codex-memory/`: untuk pengetahuan ringkas yang tahan lama seperti preferensi utente, progetto keputusan, ambiente vincolo atau
