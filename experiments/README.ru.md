...
Папка ini-dipakai для тестирования, оценки и тестирования рабочего процесса, а также для завершения проекта.

## Саран Структура

- папка satu eksperimen satu
- simpan `README.md` dengan tujuan, запуск команды, ожидаемый результат, наблюдаемый результат
- Gunakan `.env.example` был проведен эксперимент, но переменная среды